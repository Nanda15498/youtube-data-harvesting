...
image = Image.open("C:\\Users\\Vijay\\OneDrive\\Desktop\\Nanda\\YT project\\yt logo1.png")

st.image(image, caption= None)
st.title('Youtube Data Harvesting')

st.sidebar.header("Data Mining")



channel_id = st.sidebar.text_input('Enter channel Id')

Get_data = st.sidebar.button("Store in mongodb")


 #Session state to Get data button
if "variable" not in st.session_state:
    st.session_state.variable = False
if Get_data or st.session_state.variable:
    st.session_state.variable = True

    #youtube API
    api_service_name = 'youtube'
    api_version = 'v3'
    api_key = 'AIzaSyCRZNVrMgKgaAOoibvyDqnGTKquF75YsXY'
...
Migrate = st.sidebar.button('Transfer to SQL')
...
if Migrate or st.session_state_migrate_sql:
    ...
    engine = create_engine('mysql+mysqlconnector://root:123@localhost/youtube_db', echo=False)
...
connect_for_question = pymysql.connect(host='localhost', user='root', password='123', db='youtube_db')
